...
from .forms import professor_lee_7_form
...
def index(request):
    check_num = '7'
    data_list = professor_lee_7.objects.all().order_by('-id')
...
def create(request):
    ...
    if request.method == "POST":
        form = professor_lee_7_form(request.POST)
        if form.is_valid():
            post = form.save(commit=False)
            post.author = request.user
            post.save()
            return redirect('professor_lee_7:index')
    else:
        form = professor_lee_7_form()
    return render(request, 'professor/professor_lee/write.html', {'form':form, 'check_num':check_num})

def detail(request, pk):
    check_num = '7'
    notice = get_object_or_404(professor_lee_7, pk=pk)
...
def update(request,pk):
    check_num = '7'
    question = get_object_or_404(professor_lee_7, pk=pk)
    if request.method == "POST":
        form = professor_lee_7_form(request.POST,instance=question)
        if form.is_valid():
            question = form.save(commit=False)
            question.save()
            return redirect('professor_lee_7:detail', pk=question.id)
    else:
        if request.user.username == 'admin' or request.user.username == 'cemuos':
            form = professor_lee_7_form(instance=question)
            context = {
                'form': form,
                'edit': '수정하기',
                'check_num': check_num,
            ...
            return redirect('/professor_lee_7/'+str(pk))
        
@login_required(login_url='common:login')
def delete(request, pk):
    notice = professor_lee_7.objects.get(id=pk)
