...
from .forms import thesis_ma_form
from .models import thesis_ma
...
def index(request):
    ...
    data_list = thesis_ma.objects.all().order_by('-id')
    ...
    return render(request, 'thesis_ma/list.html', context)

@login_required(login_url='common:login')
def create(request):
    if request.method == "POST":
        form = thesis_ma_form(request.POST, request.FILES)
        if form.is_valid():
            ...
            post.save()
            return redirect('thesis_ma:index')
        else:
            messages.error(request, 'Error!')
            return render(request,'thesis_ma/test.html',{'form':form})
    else:
        form = thesis_ma_form()
    return render(request, 'thesis_ma/write.html', {'form':form})

def detail(request, pk):
    notice = get_object_or_404(thesis_ma, pk=pk)
    # notice = Notice.objects.filter(id=pk)
    context = {
        'notice': notice,
    }

    return render(request, 'thesis_ma/detail.html', context)

@login_required(login_url='common:login')
def update(request,pk):
    question = get_object_or_404(thesis_ma, pk=pk)
    if request.method == "POST":
        ...
        form = thesis_ma_form(request.POST, request.FILES,instance=question)
        if form.is_valid():
            question = form.save(commit=False)
            question.save()
            return redirect('thesis_ma:detail', pk=question.id)
    else:
        if request.user.username == 'admin' or request.user.username == 'cemuos':
            form = thesis_ma_form(instance=question)
            context = {
                'form': form,
                'edit': '수정하기',
            }
            ...
            return render(request, "thesis_ma/write.html", context)
        else:
            messages.error(request, "본인 게시글이 아닙니다.")
            return redirect('/thesis_ma/'+str(pk))
        
@login_required(login_url='common:login')
def delete(request, pk):
    notice = thesis_ma.objects.get(id=pk)
