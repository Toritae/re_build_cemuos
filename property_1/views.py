...
from .forms import property_1_form
...
def index(request):
    ...
    data_list = property_1.objects.all().order_by('-id')
    ...
    return render(request, 'property_1/list.html', context)

@login_required(login_url='common:login')
def create(request):
    if request.method == "POST":
        form = property_1_form(request.POST, request.FILES)
        if form.is_valid():
            ...
            post.save()
            return redirect('property_1:index')
        else:
            messages.error(request, 'Error!')
            return render(request,'property_1/test.html',{'form':form})
    else:
        form = property_1_form()
    return render(request, 'property_1/write.html', {'form':form})

def detail(request, pk):
    notice = get_object_or_404(property_1, pk=pk)
    # notice = Notice.objects.filter(id=pk)
    context = {
        'notice': notice,
    }

    return render(request, 'property_1/detail.html', context)

@login_required(login_url='common:login')
def update(request,pk):
    question = get_object_or_404(property_1, pk=pk)
    if request.method == "POST":
        ...
        form = property_1_form(request.POST, request.FILES,instance=question)
        if form.is_valid():
            question = form.save(commit=False)
            question.save()
            return redirect('property_1:detail', pk=question.id)
    else:
        if request.user.username == 'admin' or request.user.username == 'cemuos':
            form = property_1_form(instance=question)
            context = {
                'form': form,
                'edit': '수정하기',
            }
            ...
            return render(request, "property_1/write.html", context)
        else:
            messages.error(request, "본인 게시글이 아닙니다.")
            return redirect('/property_1/'+str(pk))
        
@login_required(login_url='common:login')
def delete(request, pk):
    notice = property_1.objects.get(id=pk)
