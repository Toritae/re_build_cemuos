...
from .models import member_post_alum_special
from django.contrib.auth.decorators import login_required
import os
from django.conf import settings
def index(request):
    
    data_list = member_post_alum_special.objects.all().order_by('-id')
    ...
    return render(request, 'member_board_alum_special/board_list.html', context)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            if 'photo' not in request.FILES.keys():
                return redirect('member_board_alum_special:create')
            post = form.save(commit=False)
            post.author = request.user
            post.save()
            return redirect('member_board_alum_special:index')
        else:
            messages.error(request, 'Error!')
            return render(request,'member_board_alum_special/test.html',{'form':form})
    else:
        form = PostForm()
    return render(request, 'member_board_alum_special/create.html', {'form':form})

def detail(request, pk):
    notice = get_object_or_404(member_post_alum_special, pk=pk)
    # notice = Notice.objects.filter(id=pk)
    context = {
        'notice': notice,
    }

    return render(request, 'member_board_alum_special/detail.html', context)

@login_required(login_url='common:login')
def update(request,pk):
    question = get_object_or_404(member_post_alum_special, pk=pk)
    if request.method == "POST":
        form = PostForm(request.POST, instance=question)
        if form.is_valid():
            if 'photo' not in request.FILES.keys():
                return redirect('member_board_alum_special:update',pk)
            question = form.save(commit=False)
            question.save()
            return redirect('member_board_alum_special:detail', pk=question.id)
    else:
        form = PostForm(instance=question)
    context = {'form': form, 'edit':'수정하기'}
    return render(request, 'member_board_alum_special/create.html', context)

def notice_edit_view(request, pk):
    notice = member_post_alum_special.objects.get(id=pk)
    ...
    if request.method == "POST":
        if(request.user.username == 'admin' or request.user.username == 'cemuos'):
            ...
            if form.is_valid():
                ...
                return redirect('/member_board_alum_special/'+str(pk))
    else:
        notice = member_post_alum_special.objects.get(id=pk)
